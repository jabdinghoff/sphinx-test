...
3. Edit the text like in a word processor. Type `/` on an empty line for
...
- Don't write a big "Heading 1" at the top of a page; the **Title** field
already becomes the page heading. Start with "Heading 2" and smaller.
- Some pages contain blocks like `:::{note}` or ````{toctree}`. Those are special Sphinx instructions — leave the lines starting with` :::` or

```untouched and only edit the text between them.
